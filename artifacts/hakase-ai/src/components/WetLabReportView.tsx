...
import type { MlAdmetResult, CardiotoxResult, DiliMlResult } from "@/lib/aiService";
...
export function WetLabReportView({ open, onClose, ...input }: Props) {
  const {
    ...
    simResults, extChEMBL, admetMl, cardiotoxMl, diliMl,
  } = input;
  ...
  const markdown = useMemo(
    () => (open && view === "export"
      ? buildWetLabMarkdownReport({ smiles, molName, molProps, iv, simResults, therapeuticArea, uniprotId, proteinName, pdbId, indication, admetMl, cardiotoxMl, diliMl })
      : ""),
  ...
  const handleDownload = () => {
    const md = buildWetLabMarkdownReport({ smiles, molName, molProps, iv, simResults, therapeuticArea, uniprotId, proteinName: resolvedProteinName, pdbId, indication, admetMl, cardiotoxMl, diliMl });
      ...
      <div className="flex-1 overflow-y-auto">
        {view === "export" ? (
          ...
          <div className="max-w-[980px] mx-auto px-5 py-6 space-y-4">
            ...
            {/* ── §6b ML-Enhanced ADMET — All 41 Endpoints ── */}
            {admetMl && admetMl.ml_available && (
              <Section id="ml-admet" icon={<Zap className="w-4 h-4" />} title="ML-Enhanced ADMET — All Endpoints (Chemprop GNN)">
                <div className="text-[10px] text-slate-500 mb-3">
                  ADMET-AI v2 · Chemprop-RDKit GNN ensemble · {admetMl.model_info?.endpoint_count ?? 41} endpoints ·{" "}
                  {admetMl.model_info?.inference_time_ms != null ? `${admetMl.model_info.inference_time_ms} ms` : ""}
                </div>
                {(["absorption","distribution","metabolism","excretion","toxicity"] as const).map(bucket => {
                  ...
                  return (
                    <div key={bucket} className="mb-4">
                      <div className="text-[10px] text-slate-500 uppercase tracking-wide mb-2">{bucket}</div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                        {entries.map(([key, ep]) => {
                          ...
                          const display = v == null ? "—" : typeof v === "number"
                            ...
                            : String(v);
                          const isProb = typeof v === "number" && v >= 0 && v <= 1 &&
                            (key.includes("hERG") || key.includes("AMES") || key.includes("DILI") ||
                             key.includes("ClinTox") || key.includes("Carcinogen") || key.includes("Skin") ||
                             key.includes("BBB") || key.includes("HIA") || key.includes("Pgp") ||
                             key.includes("Bioavail") || key.includes("CYP"));
                          const riskColor = isProb && typeof v === "number"
                            ? v > 0.6 ? "text-red-400" : v > 0.3 ? "text-amber-400" : "text-emerald-400"
                            : "text-white";
                          return (
                            <div key={key} className="flex justify-between items-baseline py-[3px] border-b border-white/5 text-[11px]">
                              <span className="text-slate-400">{ep.label || key}</span>
                              <span className={`font-medium ml-2 ${riskColor}`}>{display}</span>
                            </div>
                          );
                        })}
                      </div>
            ...
            {/* ── §6c hERG Paired ML Prediction ── */}
            {cardiotoxMl && (
              <Section id="ml-cardiotox" icon={<Shield className="w-4 h-4" />} title="hERG Cardiotoxicity — Paired ML Prediction">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white/[0.03] border border-white/8 rounded-lg p-3">
                    <div className="text-[10px] text-slate-500 uppercase tracking-wide mb-2">ADMET-AI (hERG_Karim, GNN)</div>
                    {admetMl?.structured?.toxicity?.["hERG"] && (
                      <div className="text-2xl font-semibold text-amber-400 mb-1">
                        {typeof admetMl.structured.toxicity["hERG"].value === "number"
                          ? admetMl.structured.toxicity["hERG"].value.toFixed(3) : "—"}
                      </div>
                    )}
                    <div className="text-[10px] text-slate-500">blocker probability</div>
                  </div>
                  <div className="bg-white/[0.03] border border-white/8 rounded-lg p-3">
                    <div className="text-[10px] text-slate-500 uppercase tracking-wide mb-2">CardioTox-RF (hERG-Wang, RF + Morgan FP)</div>
                    <div className={`text-2xl font-semibold mb-1 ${
                      cardiotoxMl.prediction.risk_class === "High" ? "text-red-400"
                      : cardiotoxMl.prediction.risk_class === "Moderate" ? "text-amber-400"
                      : "text-emerald-400"}`}>
                      {cardiotoxMl.prediction.blocker_probability?.toFixed(3) ?? "—"}
                    </div>
                    <div className="text-[10px] text-slate-500">blocker probability</div>
                    {cardiotoxMl.prediction.ic50_band && (
                      <div className="text-[10px] text-slate-400 mt-1">IC₅₀ band: {cardiotoxMl.prediction.ic50_band}</div>
                    )}
                  </div>
                </div>
                <div className="mt-3 text-[10px] text-slate-500 italic">
                  Two independent models — different training sets, algorithms, and feature representations — for a more robust safety call.
                </div>
              </Section>
            )}

            {/* ── §6d DILI ML Prediction ── */}
            {diliMl && (
              <Section id="ml-dili" icon={<AlertTriangle className="w-4 h-4" />} title="DILI — Drug-Induced Liver Injury (ML)"
                tag={<RiskBadge level={diliMl.prediction.risk_class ?? "Moderate"} />}>
                <DataRow label="DILI probability" value={diliMl.prediction.dili_probability?.toFixed(3) ?? "—"}
                  badge={diliMl.prediction.risk_class ?? undefined} highlight prov="ml" />
                <DataRow label="Risk class" value={diliMl.prediction.risk_class ?? "—"} />
                {diliMl.prediction.severity_hint && (
                  <DataRow label="Severity hint" value={diliMl.prediction.severity_hint} />
                )}
                {diliMl.model_info?.engine && (
                  <DataRow label="Model" value={diliMl.model_info.engine} />
                )}
                <div className="mt-2 text-[10px] text-slate-500">
                  Trained on FDA DILIrank dataset. DILI probability &gt; 0.5 = concern; &gt; 0.7 = high risk.
                </div>
              </Section>
            )}
