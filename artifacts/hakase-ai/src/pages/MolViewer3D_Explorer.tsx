...
function MiniMol({ rx, ry, color, liveAtoms, liveBonds }: { rx: number; ry: number; color: string; liveAtoms?: typeof BASE_ATOMS; liveBonds?: typeof BASE_BONDS }) {
  const cx = 55, cy = 45;
  const srcAtoms = liveAtoms ?? BASE_ATOMS;
  const srcBonds = liveBonds ?? BASE_BONDS;
  const scale = liveAtoms ? 0.20 : 0.55;
  const atoms = srcAtoms.slice(0, liveAtoms ? srcAtoms.length : 8).map(a => {
    const r = rot(liveAtoms ? a.x : a.x - 40, liveAtoms ? a.y : a.y - 20, a.z, rx, ry);
    return { ...a, px: cx + r.x * scale, py: cy + r.y * scale, pz: r.z };
  }).sort((a, b) => a.pz - b.pz);
  const pm = Object.fromEntries(atoms.map(a => [a.id, a]));

  return (
    <svg width="110" height="90">
      {(liveAtoms ? srcBonds : srcBonds.slice(0, 9)).map((b, i) => {
...
function BigMol({ rx, ry, zoom, color, dynAtoms, dynBonds }: { rx: number; ry: number; zoom: number; color: string; dynAtoms: typeof BASE_ATOMS; dynBonds: typeof BASE_BONDS }) {
  const cx = 200, cy = 160;
  const atoms = dynAtoms.map(a => {
...
function ExplorerInteractiveStage({
  rx, ry, zoom, setRx, setRy, setZoom, children,
}: {
  ...
  return (
    <div
      {...bind}
      style={{ ...bind.style, transform: `scale(${zoom})`, cursor: isDragging ? "grabbing" : "grab" }}
...
export function MolViewer3D_Explorer({ smiles, molName: propMolName }: { smiles?: string; molName?: string } = {}) {
  ...
  const dynAtoms = (mol3d?.atoms ?? BASE_ATOMS) as typeof BASE_ATOMS;
  const dynBonds = (mol3d?.bonds ?? BASE_BONDS) as typeof BASE_BONDS;
  const dynFormula = mol3d?.formula;
  const dynMolName = propMolName || "Molecule";

  const liveMolecule: MoleculeCard = useMemo(() => ({
    id: "live",
    name: dynMolName,
    formula: dynFormula ?? "—",
    mw: 0,
    ...
    ryOffset: 0.3,
    accentColor: "#10b981",
  }), [dynMolName, dynFormula]);

  const allMolecules = useMemo(() => [liveMolecule, ...MOLECULES], [liveMolecule]);
  ...
  const primaryMol = allMolecules.find(m => m.id === primary) ?? allMolecules[0];

  const filtered = allMolecules
    .filter(m => {
    ...
    <div className="h-screen bg-[#080a10] text-white flex flex-col overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="h-12 border-b border-white/8 bg-[#0c0e18] flex items-center px-4 gap-3 shrink-0">
        <div className="flex items-center gap-2 pr-3 border-r border-white/8">
          ...
          <span className="text-[13px] font-bold">Molecule Explorer</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
                  ...
                  >
                    ...
                    <div className="bg-black/30 flex items-center justify-center pt-2 pb-1 overflow-hidden">
                      <MiniMol rx={mol.rxOffset} ry={mol.ryOffset + ry * 0.3} color={mol.accentColor}
                        liveAtoms={mol.id === "live" && mol3d ? dynAtoms : undefined}
                        liveBonds={mol.id === "live" && mol3d ? dynBonds : undefined} />
                    </div>

                    <div className="px-2 pb-2">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[11px] font-semibold text-slate-200 truncate flex-1">{mol.name}</span>
                        {mol.id === "live"
                          ? <span className="text-[8px] text-emerald-400 border border-emerald-500/30 px-1 py-0 rounded shrink-0 ml-1">LIVE</span>
                          : mol.starred && <Star className="w-3 h-3 text-amber-400 fill-amber-400 shrink-0" />
                        }
                      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {mol.id === "live"
                          ? <span className="text-[8px] text-emerald-400 border border-emerald-500/30 px-1 py-0 rounded">LIVE</span>
                          : mol.tag && <Badge className="text-[8px] px-1 py-0 bg-white/5 text-slate-500">{mol.tag}</Badge>}
                      </div>
                      <div className="text-[9px] text-slate-500 font-mono">{mol.formula}{mol.mw > 0 ? ` · ${mol.mw} Da` : ""}</div>
                    </div>
        ...
        <div className="flex-1 flex flex-col relative bg-[#070810]">
          ...
          <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: mol3d ? "#10b981" : primaryMol.accentColor }} />
            <span className="text-[13px] font-semibold text-white">{mol3d ? dynMolName : primaryMol.name}</span>
            <span className="text-[11px] text-slate-500 font-mono">{mol3d ? dynFormula : primaryMol.formula}</span>
            {mol3d && <span className="text-[9px] text-emerald-400/60 border border-emerald-500/20 px-1.5 py-0.5 rounded">● Live from SMILES</span>}
            {!mol3d && primaryMol.tag && (
          ...
          <div className="flex-1 flex items-center justify-center relative z-10">
            <ExplorerInteractiveStage
              rx={rx} ry={ry} zoom={zoom}
              setRx={setRx} setRy={setRy} setZoom={setZoom}
            >
              <BigMol rx={rx} ry={ry} zoom={1} color={mol3d ? "#10b981" : primaryMol.accentColor} dynAtoms={dynAtoms} dynBonds={dynBonds} />
            </ExplorerInteractiveStage>
          </div>
                  ...
                  <div className="bg-black/30 rounded-xl p-4 flex items-center justify-center">
                    <svg width="140" height="130">
                      {(() => {
                        ...
                        const paths = selected.slice(0, 3).flatMap(sid => {
                          const mol = MOLECULES.find(m => m.id === sid);
                          if (!mol) return [];
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-4">
                ...
                <div className="border-t border-white/8 pt-3">
                  ...
                  <div className="space-y-1.5">
                    {(() => {
                      const isLive = primaryMol.id === "live";
                      const p = isLive ? molProps : null;
                      return [
                        { k: "Molecular Weight", v: p ? `${p.mw} Da` : `${primaryMol.mw} Da` },
                        { k: "Binding Affinity", v: primaryMol.binding },
                        { k: "Toxicity Risk", v: p ? (p.tpsa > 120 ? "High" : p.tpsa > 80 ? "Medium" : "Low") : primaryMol.tox },
                        { k: "Synthetic Accessibility", v: `${primaryMol.sa}/10` },
                        { k: "H-Bond Donors", v: p ? String(p.hbd) : "—" },
                        { k: "H-Bond Acceptors", v: p ? String(p.hba) : "—" },
                        { k: "Rotatable Bonds", v: p ? String(p.rotBonds) : "—" },
                        { k: "logP", v: p ? String(p.logP) : "—" },
                      ];
                    })().map(({ k, v }) => (
